...
/// Build 26 — Memory-optimized for Network Extension ~50MB limit.
///
/// Key optimizations vs build 24:
/// - Socket buffers: 2MB×4 → 128KB×4 (saves ~7.5MB)
/// - Pre-allocated reusable read buffer (no per-event alloc/dealloc)
/// - Zero-copy packet framing with pre-built headers
/// - Batch writePackets calls from tun2socks
/// - Memory pressure watchdog: drops packets when available < 15MB
/// - Buffered file logging (no synchronizeFile per line)
/// - Reduced hev-socks5-tunnel stack size
/// - MTU 1400 (smaller packets = less peak memory)
class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    // Pre-allocated read buffer for tun2socks → packetFlow relay (reused across events)
    private let readBufSize = 2048
    private var readBuf: UnsafeMutablePointer<UInt8>?

    // Pre-built AF headers to avoid per-packet Data creation
    ...
    private let logFlushInterval: TimeInterval = 5
    ...
    private func setupFileLogging() {
        ...
        // Flush log buffer periodically instead of on every line
        let timer = DispatchSource.makeTimerSource(queue: DispatchQueue.global(qos: .utility))
        timer.schedule(deadline: .now() + logFlushInterval, repeating: logFlushInterval)
        ...
        timer.setEventHandler { [weak self] in
            ...
            if m.avail < 15.0 && m.avail > 0 {
                if !self.isUnderMemoryPressure {
                    self.fileLog("WARNING: entering memory pressure mode (avail=\(String(format: "%.1f", m.avail))MB)")
                    self.isUnderMemoryPressure = true
                }
            } else if m.avail > 20.0 {
                if self.isUnderMemoryPressure {
                    self.fileLog("Memory pressure relieved (avail=\(String(format: "%.1f", m.avail))MB)")
    ...
    override func startTunnel(options: [String: NSObject]?, completionHandler: @escaping (Error?) -> Void) {
        setupFileLogging()
        let m0 = getMemoryMB()
        fileLog("Starting tunnel (build 26) — memory-optimized")
        ...
        settings.mtu = 1400  // Smaller MTU = smaller packets = less peak memory
    ...
    private func startTun2Socks(socksPort: Int) {
        ...
        // Small socket buffers — 128KB each instead of 2MB (saves ~7.5MB total)
        ...
        // Reduced task-stack-size for lower memory footprint
        let config = """
        tunnel:
          mtu: 1400
        socks5:
    ...
    /// packetFlow → tun2socks: read IP packets from iOS and write to socketpair
    private func startReadingFromPacketFlow() {
        packetFlow.readPackets { [weak self] packets, protocols in
            guard let self = self, self.isRelayRunning else { return }

            // Under memory pressure: drop packets to stay alive
            if self.isUnderMemoryPressure {
            ...
            for (i, packet) in packets.enumerated() {
                ...
                // Write AF header + packet in a single contiguous buffer
                ...
                let written = packet.withUnsafeBytes { packetPtr -> Int in
                    ...
                    return afHeader.withUnsafeBytes { headerPtr -> Int in
                        ...
                        // Use iovec for scatter-gather write to avoid Data copy
                        var iov = [
    ...
    /// tun2socks → packetFlow: read framed packets from socketpair and inject into iOS
    private func startReadingFromTun2Socks() {
        ...
        source.setEventHandler { [weak self] in
            ...
            // Batch packets for writePackets (fewer calls = less overhead)
            ...
            while true {
                ...
                // Under memory pressure: drop incoming packets
                if self.isUnderMemoryPressure {
                    self.packetsDropped += 1
                    continue
                }
                ...
                batchPackets.append(ipPacket)
                batchProtos.append(proto)

                // Flush batch at 32 packets to bound memory
